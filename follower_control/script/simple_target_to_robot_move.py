...
class FollowerControl:
    def __init__(self):
        self.movement_mode = rospy.get_param("/follower_control/movement_mode", "const")
        self.twist_mul = rospy.get_param("/follower_control/twist_mul", 1.0)
        self.max_fwd = rospy.get_param("/follower_control/max_fwd", 1.0) # m/s
        self.max_rot = rospy.get_param("/follower_control/max_rot", 0.8) # rad/s
        # self.max_fwd_rpm = rospy.get_param("/follower_control/max_fwd_rpm", 180)
        # self.max_rot_rpm = rospy.get_param("/follower_control/max_rot_rpm", 70)
        self.kp_fwd = rospy.get_param("/follower_control/kp_fwd", 0.1)
        self.kp_rot = rospy.get_param("/follower_control/kp_rot", 0.3)
        self.err_fwd = rospy.get_param("/follower_control/err_fwd", 1)
        self.err_rot = rospy.get_param("/follower_control/err_rot", 0.5)

        self.wheel_radius = rospy.get_param("/follower_control/wheel_radius", 2.75) / 100.0
        self.wheel_distance = rospy.get_param("/follower_control/wheel_distance", 23.0) / 100.0
        self.gear_ratio = rospy.get_param("/follower_control/gear_ratio", 1.0)
    ...
    def target_to_motor_rpm(self, target : PoseStamped) -> Union[int, int]:
        ...
        self.max_fwd_rpm *= 200 # 200 is a magic number!
